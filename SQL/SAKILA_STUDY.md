...
## 🔍 Basic To Advanced Query Scenarios

### 1. Basic Data Retrieval
**Goal:** Preview the movie collection.
```sql
SELECT * 
FROM film
LIMIT 10;
```
### 2. Sorting & Selecting Columns
**Goal:** Get an alphabetical list of actors by their last names.
```sql
...
**Goal:** Find the description of a specific movie title.
```sql
...
```
### 4. Range Testing
**Goal:** List movies with a duration between 120 and 150 minutes.
```sql
SELECT title, length
FROM film
...
**Goal:** Find all actors whose first name starts with the letter 'J'.
```sql
SELECT first_name, last_name
FROM actor
...
```
### 6. Data Aggregation
**Goal:** Count the total number of films in the inventory.
```sql
SELECT COUNT(*)
FROM film;
```
### 7. Grouping & Distribution
**Goal:** See the distribution of movies across different age ratings (PG, R, etc.).
```sql
SELECT rating, count(*)
FROM film
GROUP BY rating;
```
### 8. Basic Table Join
**Goal:** Retrieve a list of customers and their corresponding street addresses.
```sql
SELECT c.first_name, c.last_name, a.address
FROM customer c
JOIN address a ON a.address_id = c.address_id;
```
### 9. Multi-Table Relational Join
**Goal:** Display movie titles alongside their respective categories.
```sql
...
```
### 10. Advanced Filtering (Aggregation + Having)
**Goal:** Identify high-value customers who have spent more than $150 in total.
```sql
...
```
### 11. Multi-Table Data Retrieval (JOINs)
...
### 12. Identifying Customer Activity (Aggregation)
...
### 13. Inventory Availability Analysis
...
### 14. Cross-Relational Query (Customer Rental History)
